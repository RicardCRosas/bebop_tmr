...
class MissionAdvanceNode:
    def __init__(self):
        ...
        self.max_goal_exec_time = rospy.get_param("~max_goal_exec_time", 20.0)
    ...
    def execute_forward_to_goal(self):
        ...
        while not rospy.is_shutdown():
            ...
            if remaining <= self.goal_tolerance:
                ...
                self.do_hold_then_land(self.goal_hold_time, x_hold, y_hold, z_hold, yaw_hold)
                return True
            ...
            if rospy.Time.now().to_sec() - start_time > self.max_goal_exec_time:
                rospy.logwarn("Timeout avanzando hacia /goal_pose.")
                self.publish_zero_cmd_burst()
                return False
